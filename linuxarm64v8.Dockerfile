...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates autoconf automake build-essential gettext git libtool python3 python3-mako wget gnupg dirmngr git \
  autoconf-archive libc6-arm64-cross gcc-aarch64-linux-gnu g++-aarch64-linux-gnu pkg-config libev-dev libcurl4-gnutls-dev libsqlite3-dev
...
## Install sparko
RUN curl https://github.com/fiatjaf/sparko/releases/download/v2.8/sparko_linux_arm64 --output /root/sparko_linux_arm64 && \
    chmod +x /root/sparko_linux_arm64
